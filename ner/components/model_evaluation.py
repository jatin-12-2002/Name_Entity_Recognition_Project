    ...
    def initiate_model_evaluation(self) -> ModelEvaluationArtifacts:
        try:
            ...
            if os.path.exists(self.model_evaluation_config.aws_local_path) == True:
                logging.info("AWS S3 model file available in the root directory")

                aws_model = torch.load(self.model_evaluation_config.aws_local_path, map_location=torch.device('cpu'))
                logging.info("AWS S3 model loaded")

                aws_model_accuracy = self.evaluate(model=aws_model, df_test=df_test)
                logging.info(
                    f"Calculated the aws S3 model's Test accuracy. - {aws_model_accuracy}"
                )
                ...
                logging.info("AWS S3 model is not available locally for comparison.")
