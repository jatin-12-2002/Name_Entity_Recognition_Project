    ...
    def initiate_data_transformation(self) -> DataTransformationArtifacts:
        ...
        try:
            ...
            self.awscloud.upload_file(
                ...
                from_filename=self.data_transformation_config.ids_to_labels_path,
                to_filename=IDS_TO_LABELS_FILE_NAME,
                remove=False
            )
            logging.info(
                f"Uploaded the ids to labels pickle file to AWS S3 cloud storage. File name - {os.path.basename(self.data_transformation_config.ids_to_labels_path)}"
            )
