...
class ModelPredictor:
    ...
    def initiate_model_predictor(self, sentence: str) -> str:
        ...
        try:
            ...
            logging.info("Tokenizer Pickle file downloaded from AWS S3 storage")
            ...
            logging.info("ids to label Pickle file downloaded from AWS S3 storage")

            self.awscloud.download_object(
                bucket_name=BUCKET_NAME,
                key=AWS_MODEL_NAME,
                filename=os.path.join(self.model_predictor_config.best_model_from_aws_path,AWS_MODEL_NAME)
            )
